...
class CourseClass:
    def __init__(self,root):
        ...
        #===title===
        title = Label(self.root, text="Manage Course Details", font=("goudy old style", 20, "bold"), bg="#040c76", fg="white").place(x=10, y=15, width=1180, height=35)
        #variables====
        ...
        #===buttons===
        self.btn_add=Button(self.root,text='Save',font=("goudy old style",15,"bold"),bg="#008cff",fg="white",cursor="hand2",command=self.add)
        self.btn_add.place(x=150,y=400,width=110,height=40)
        self.btn_update=Button(self.root,text='Update',font=("goudy old style",15,"bold"),bg="#3ea800",fg="white",cursor="hand2",command=self.update)
        self.btn_update.place(x=270,y=400,width=110,height=40)
        self.btn_delete=Button(self.root,text='Delete',font=("goudy old style",15,"bold"),bg="#ff0000",fg="white",cursor="hand2",command=self.delete)
        self.btn_delete.place(x=390,y=400,width=110,height=40)
        self.btn_clear=Button(self.root,text='Clear',font=("goudy old style",15,"bold"),bg="#565656",fg="white",cursor="hand2",command=self.clear)
        ...
        btn_search=Button(self.root,text='Search',font=("goudy old style",15,"bold"),bg="#008cff",fg="white",cursor="hand2",command=self.search).place(x=1070,y=60,width=120,height=28)
        #===Content===
        self.C_Frame=Frame(root,bd=2,relief=RIDGE)
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succcess","Course Added Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succcess","Course update Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
