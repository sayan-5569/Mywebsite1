...
class resultclass:
    def __init__(self,root):
        ...
        #===title===
        title = Label(self.root, text="Add Student Result", font=("goudy old style", 20, "bold"), bg="#040c76", fg="#FFFFFF").place(x=10, y=15, width=1180, height=50)
        #====widgets=======
        #====Variables===========
        ...
        lbl_course=Label(self.root,text="Department",font=("goudy old style",20,'bold'),bg='white').place(x=50,y=220)
        lbl_marks_obj=Label(self.root,text="Marks Obtained",font=("goudy old style",20,'bold'),bg='white').place(x=50,y=280)
        ...
        btn_search=Button(self.root,text='Search',font=("goudy old style",15,"bold"),bg="#008cff",fg="white",cursor="hand2",command=self.search).place(x=500,y=100,width=100,height=28)
        ...
        #=====button=======
        btn_add=Button(self.root,text="Submit",font=("times new roman",15),bg="#40ff00",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300,y=420,width=120,height=35)
        ...
        try:
            if self.var_name.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succcess","Result Added Successfully",parent=self.root)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
    def clear(self):
