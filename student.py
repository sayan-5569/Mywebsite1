...
class StudentClass:
    def __init__(self,root):
        ...
        #===title===
        title = Label(self.root, text="Manage Student Details", font=("goudy old style", 20, "bold"), bg="#040c76", fg="white")
        ...
        #===Entry fields===
        self.course_list=[]
        #====function call to uppdate the list======
        self.fetch_course()
    ...
    def add(self):
        try:
            if self.var_roll.get() == "":
                ...
                with sqlite3.connect(database="ams.db") as con:
                    cur = con.cursor()
                    cur.execute("SELECT * FROM student WHERE roll=?", (self.var_roll.get(),))
                    row = cur.fetchone()
                    if row is not None:
        ...
        try:
            if self.var_roll.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succcess","Student update Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
